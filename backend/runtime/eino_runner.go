...
func NewEinoRunner(ctx context.Context, llmConfig *config.LLMConfig, runtimeConfig Config) (*EinoRunner, error) {
	...
	return &EinoRunner{
		chatModel:    chatModel,
		toolAdapter:  runtimeeino.NewToolAdapter(runtimeConfig.ToolRegistry, runtimeConfig.ToolRuntime),
...
func (r *EinoRunner) HandleEvent(ctx context.Context, event *interaction.Event) error {
	...
	agentRunner, err := runtimeeino.NewAgentRunner(ctx, &runtimeeino.AgentRunnerConfig{
		Model:        r.chatModel,
		ToolAdapter:  r.toolAdapter,
		Binding:      runtimeeino.ToolBinding{Selector: authSelectorFromEvent(event), UserID: event.Actor},
